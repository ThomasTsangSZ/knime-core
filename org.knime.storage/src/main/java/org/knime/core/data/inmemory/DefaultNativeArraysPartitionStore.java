...
class DefaultNativeArraysPartitionStore<T extends NativeArray<?>> implements NativeArraysPartitionStore<T> {
	...
	public DefaultNativeArraysPartitionStore(Supplier<PartitionValue<T>> partitionValueSupplier,
	...
	public Partition<T> createPartition() {
		...
		return new NativeArraysPartition<T>(t, t.size(), m_partitionIdx++) {
